                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($inhumations as $inhumation)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $inhumation->personne->nom ?? 'N/A' }} {{ $inhumation->personne->postnom ?? ' -' }} {{ $inhumation->personne->prenom ?? '-' }} {{ $inhumation->personne->date_naissance->format('d/m/Y') ?? ' -' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $inhumation->date_inhumation->format('d/m/Y') ?? 'N/A' }}</td>
