    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">
                👶 Détails de l'acte d'inhumation
            </h2>
            
            <div class="flex items-center gap-3">
                <!-- Boutons d'action rapide -->
                <a href="{{ route('inhumations.attestation', $inhumation  ) }}" 
                   target="_blank"
                   class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 transition">
                    👁️ Voir attestation
                </a>
                
                <a href="{{ route('inhumations.attestation.pdf', $inhumation) }}"
                ...
                <a href="{{ route('inhumations.edit', $inhumation) }}" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex justify-between items-center flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    <div class="bg-sky-600 text-white px-4 py-2 rounded-lg font-bold">
                        Acte N° {{ str_pad($inhumation->id, 6, '0', STR_PAD_LEFT) }}
                    </div>
                    <div class="text-sm text-gray-600">
                        <span class="font-semibold">Enregistré le :</span> {{ $inhumation->created_at->format('d/m/Y à H:i') }}
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    @php
                        $numeroOfficiel = $inhumation->numero_acte ?? 'INH-' . date('Y') . '-' . str_pad($inhumation->id, 4, '0', STR_PAD_LEFT);
                    @endphp
                    ...
                    <!-- Statut de l'acte -->
                    @php
                        $statutClass = match($inhumation->statut ?? 'enregistre') {
                            'enregistre' => 'bg-green-100 text-green-800',
                            'en_attente' => 'bg-yellow-100 text-yellow-800',
                            'annule' => 'bg-red-100 text-red-800',
                            default => 'bg-gray-100 text-gray-800'
                        };
                        $statutLabel = match($inhumation->statut ?? 'enregistre') {
                            'enregistre' => '✅ Enregistré',
                            'en_attente' => '⏳ En attente',
                            'annule' => '❌ Annulé',
                            default => '📄 ' . ($inhumation->statut ?? 'Enregistré')
                        };
                    @endphp
                ...
                <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                ...
                                <span class="font-semibold text-gray-800 text-lg">
                                    {{ $inhumation->personne->nom }} {{ $inhumation->personne->prenom }} 
                                    @if($inhumation->personne->postnom)
                                        <span class="text-gray-600 font-normal">{{ $inhumation->personne->postnom }}</span>
                                    @endif
                                </span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                ...
                                <span class="text-gray-700">
                                    {{ $inhumation->personne->date_naissance ? \Carbon\Carbon::parse($inhumation->personne->date_naissance)->format('d/m/Y') : 'Non renseignée' }}
                                </span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Heure de naissance :</span>
                                <span class="text-gray-700">{{ $inhumation->personne->heure_naissance ?? 'Non renseignée' }}</span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Lieu de naissance :</span>
                                <span class="text-gray-700">{{ $inhumation->personne->lieu_naissance ?? $inhumation->lieu_naissance ?? 'Non renseigné' }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-500 font-medium">Sexe :</span>
                                <span class="px-3 py-1 {{ $inhumation->personne->sexe == 'M' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }} rounded-full text-sm font-medium">
                                    {{ $inhumation->personne->sexe == 'M' ? 'Masculin' : 'Féminin' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-4 bg-gradient-to-r from-gray-50 to-sky-50 rounded-xl border border-gray-200">
                            ...
                            <div class="space-y-2 text-sm">
                                <div class="flex items-start gap-2">
                                    ...
                                    <span class="font-medium text-gray-800">{{ $inhumation->etablissement ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Localité :</span>
                                    <span class="font-medium text-gray-800">{{ $inhumation->localite->nom ?? $inhumation->personne->lieu_naissance ?? 'Non renseignée' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Commune/Secteur :</span>
                                    <span class="font-medium text-gray-800">{{ $inhumation->commune ?? $inhumation->personne->secteur->nom ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Territoire :</span>
                                    <span class="font-medium text-gray-800">{{ $inhumation->territoire->nom ?? $inhumation->personne->territoire->nom ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Province :</span>
                                    <span class="font-medium text-gray-800">{{ $inhumation->province->nom ?? $inhumation->personne->province->nom ?? 'Non renseignée' }}</span>
                                </div>
                    ...
                    <div class="flex flex-col items-center justify-start">
                        ...
                        <div class="mt-4 w-full max-w-xs">
                            <div class="bg-sky-600 text-white rounded-lg p-3 text-center">
                                ...
                                <div class="text-xs mt-1 opacity-80">
                                    Déclaré le {{ $inhumation->date_declaration ? \Carbon\Carbon::parse($inhumation->date_declaration)->format('d/m/Y') : $inhumation->created_at->format('d/m/Y') }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-3 w-full max-w-xs bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                            ...
                            <p class="text-sm text-gray-700">{{ $inhumation->mention_marginale }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="font-semibold text-gray-800">
                                {{ $inhumation->pere_nom ?? $inhumation->personne->pere ?? 'Non renseigné' }}
                            </div>
                            @if($inhumation->pere_profession)
                                <div class="text-xs text-gray-500 mt-1">{{ $inhumation->pere_profession }}</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="font-semibold text-gray-800">
                                {{ $inhumation->mere_nom ?? $inhumation->personne->mere ?? 'Non renseignée' }}
                            </div>
                            @if($inhumation->mere_profession)
                                <div class="text-xs text-gray-500 mt-1">{{ $inhumation->mere_profession }}</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="pb-3 border-b border-gray-100">
                        ...
                        <p class="font-semibold text-gray-800 mt-1">
                            {{ $inhumation->declarant_nom ?? 'Non renseigné' }}
                        </p>
                        @if($inhumation->declarant_qualite)
                            <p class="text-sm text-gray-500">{{ $inhumation->declarant_qualite }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        ...
                        <span class="font-semibold">{{ $inhumation->entite->nom ?? 'Non définie' }}</span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        <span class="text-gray-500 font-medium">Officier d'état civil :</span>
                        <span class="font-medium">{{ $inhumation->officier_nom ?? $inhumation->user->name ?? 'Inconnu' }}</span>
                    </div>
                    <div class="flex justify-between items-center pt-2">
                        <span class="text-gray-500 font-medium">Dernière modification :</span>
                        <span class="text-sm text-gray-600">{{ $inhumation->updated_at->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white rounded-xl shadow p-4 border-l-4 border-indigo-500">
                ...
                <p class="font-bold text-gray-800">{{ $inhumation->annee_enregistrement ?? date('Y') }}</p>
            </div>
        </div>

        <!-- SECTION PIÈCES JOINTES -->
        @if($inhumation->documents || $inhumation->certificat_medical || $inhumation->personne->photo)
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @if($inhumation->certificat_medical)
                        <div class="group relative">
                            <div class="relative overflow-hidden rounded-lg bg-gray-100 aspect-square">
                                <img src="{{ asset('storage/' . $inhumation->certificat_medical) }}" 
                    ...
                    @if($inhumation->documents)
                        <div class="group relative">
                            <div class="relative overflow-hidden rounded-lg bg-gray-100 aspect-square">
                                <img src="{{ asset('storage/' . $inhumation->documents) }}" 
                                     alt="Document"
                                     class="w-full h-full object-cover">
                            </div>
        ...
        <div class="mt-6 text-right text-xs text-gray-400">
            Acte dressé le {{ $inhumation->created_at->format('d/m/Y à H:i') }} 
            • Dernière mise à jour {{ $inhumation->updated_at->diffForHumans() }}
        </div>
    </div>
</x-app-layout>