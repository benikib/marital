...
<head>
    ...
    <title>Acte de celibat - RDC</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="acte">
        ...
        <div class="header">
            ...
            <div class="title">ACTE DE CELIBAT</div>
            <div class="numero">
                N° {{ $celibat->numero_acte ?? 'CEL-' . date('Y') . '-' . str_pad($celibat->id, 5, '0', STR_PAD_LEFT) }}
            </div>
        </div>
        ...
        <div class="content">
            ...
            <div class="identity-block">
                <div class="identity-row">
                    ...
                    <span class="value">
                        <strong>{{ strtoupper($celibat->personne->nom) }}</strong> 
                        {{ $celibat->personne->prenom }}
                        @if($celibat->personne->postnom)
                            {{ $celibat->personne->postnom }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    <span class="label">Sexe :</span>
                    <span class="value">{{ $celibat->personne->sexe == 'M' ? 'Masculin' : 'Féminin' }}</span>
                </div>
                <div class="identity-row">
                    <span class="label">Né(e) le :</span>
                    <span class="value">
                        {{ $celibat->personne->date_naissance ? \Carbon\Carbon::parse($celibat->personne->date_naissance)->translatedFormat('d F Y') : '_________' }}
                        @if($celibat->heure_naissance)
                            à {{ $celibat->heure_naissance }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    ...
                    <span class="value">
                        {{ $celibat->personne->lieu_naissance ?? $celibat->lieu_naissance ?? '_________________' }}
                        @if($celibat->etablissement)
                            ({{ $celibat->etablissement }})
                        @endif
                    </span>
                </div>
            </div>
            ...
            <div class="parents-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $celibat->pere_profession }}</span>
                </div>
                @endif
                <div class="identity-row" style="margin-top: 8px;">
                    <span class="label">Mère :</span>
                    <span class="value">{{ $celibat->mere_nom ?? $celibat->personne->mere ?? '_________________' }}</span>
                </div>
                @if($celibat->mere_profession)
                <div class="identity-row">
                    <span class="label">Profession :</span>
                    <span class="value">{{ $celibat->mere_profession }}</span>
                </div>
                @endif
            </div>
            ...
            <div class="declaration-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $celibat->declarant_qualite }}</span>
                </div>
                @endif
                <div class="identity-row">
            ...
            <div style="display: flex; gap: 15px; margin: 8px 0; font-size: 11px;">
                <div>
                    ...
                    {{ $celibat->type_naissance ?? 'Simple' }}
                </div>
                <div>
                    <span style="font-weight: 700;">Registre N° :</span> 
                    {{ $celibat->numero_registre ?? '________' }}
                </div>
            </div>
            ...
            <div class="mention-box">
                <strong>📌 Mention marginale :</strong> {{ $celibat->mention_marginale }}
            </div>
            @endif
            ...
            <div class="attestation-text">
                <p>
                    ...
                    <strong>{{ $celibat->entite->nom ?? '_________________' }}</strong>.
                </p>
            </div>
        ...
        <div class="signature-section">
            <div class="date-place">
                <p>Fait à {{ $celibat->entite->nom ?? 'Kinshasa' }},</p>
            ...
            <div class="signature">
                ...
                <div style="margin-top: 2px; font-size: 10px;">
                    {{ $celibat->officier_nom ?? $celibat->user->name ?? '_______________' }}
                </div>
                <div class="seal">
        ...
        <div class="qr-section">
            <div class="qr-code">
                <img width="65" height="65" 
                     src="https://api.qrserver.com/v1/create-qr-code/?size=75x75&data={{ urlencode(route('celibats.verify', $celibat)) }}&bgcolor=fffdf8&color=0284c7" 
                     alt="QR">
            </div>
            <div class="qr-text">
                ...
                <p style="margin-top: 3px; font-size: 7px;">
                    Réf : {{ $celibat->numero_acte ?? 'CEL-' . $celibat->id }}
                </p>
            </div>
        </div>
        ...
        <!-- Filigrane -->
        <div class="watermark">ACTE DE CÉLIBAT</div>
    </div>
</body>
</html>