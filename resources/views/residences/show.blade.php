    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('residences.attestation', $residence) }}" target="_blank"
            ...
            <div class="flex justify-between items-center flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div class="text-sm text-gray-600">
                        <span class="font-semibold">Enregistré le :</span>
                        {{ $residence->created_at->format('d/m/Y à H:i') }}
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    @php
                        $numeroOfficiel =
                            $residence->numero_acte ??
                            'RES-' . date('Y') . '-' . str_pad($residence->id, 4, '0', STR_PAD_LEFT);
                    @endphp
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-3">
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                ...
                                <span class="font-semibold text-gray-800 text-lg">
                                    ...
                                    @if ($residence->personne->postnom)
                                        <span
                                            class="text-gray-600 font-normal">{{ $residence->personne->postnom }}</span>
                                    @endif
                                </span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                            ...
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                ...
                                <span
                                    class="text-gray-700">{{ $residence->heure_naissance ?? 'Non renseignée' }}</span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Lieu de naissance :</span>
                                <span
                                    class="text-gray-700">{{ $residence->personne->lieu_naissance ?? ($residence->lieu_naissance ?? 'Non renseigné') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-500 font-medium">Sexe :</span>
                                <span
                                    class="px-3 py-1 {{ $residence->personne->sexe == 'M' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }} rounded-full text-sm font-medium">
                        ...
                        <div class="mt-4 p-4 bg-gradient-to-r from-gray-50 to-sky-50 rounded-xl border border-gray-200">
                            ...
                            <div class="space-y-2 text-sm">
                                <div class="flex items-start gap-2">
                                    ...
                                    <span
                                        class="font-medium text-gray-800">{{ $residence->etablissement ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Localité :</span>
                                    <span
                                        class="font-medium text-gray-800">{{ $residence->localite->nom ?? ($residence->personne->lieu_naissance ?? 'Non renseignée') }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Commune/Secteur :</span>
                                    <span
                                        class="font-medium text-gray-800">{{ $residence->commune ?? ($residence->personne->secteur->nom ?? 'Non renseigné') }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Territoire :</span>
                                    <span
                                        class="font-medium text-gray-800">{{ $residence->territoire->nom ?? ($residence->personne->territoire->nom ?? 'Non renseigné') }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Province :</span>
                                    <span
                                        class="font-medium text-gray-800">{{ $residence->province->nom ?? ($residence->personne->province->nom ?? 'Non renseignée') }}</span>
                                </div>
                    ...
                    <div class="flex flex-col items-center justify-start">
                        @if ($residence->personne->photo)
                            <div
                                class="relative w-48 h-48 rounded-xl overflow-hidden shadow-lg border-2 border-gray-200">
                                <img src="{{ asset('storage/' . $residence->personne->photo) }}" alt="Photo d'identité"
                                    class="w-full h-full object-cover">
                            </div>
                            <p class="mt-2 text-xs text-gray-500">Photo d'identité</p>
                        @else
                            <div
                                class="w-48 h-48 rounded-xl bg-gray-100 border-2 border-dashed border-gray-300 flex flex-col items-center justify-center gap-2">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        ...
                        <span
                            class="font-medium">{{ $residence->officier_nom ?? ($residence->user->name ?? 'Inconnu') }}</span>
                    </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        @if ($residence->personne->photo)
                            <div class="group relative">
                                <div class="relative overflow-hidden rounded-lg bg-gray-100 aspect-square">
                                    <img src="{{ asset('storage/' . $residence->personne->photo) }}" alt="Photo"
                                        class="w-full h-full object-cover">
                                </div>
