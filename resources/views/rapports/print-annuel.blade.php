...
<head>
    ...
    <title>Rapport Annuel des Mariages - {{ $annee }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>Rapport Annuel des Mariages</h1>
        <p>Année : {{ $annee }}</p>
    ...
    <div class="section">
        <div class="section-title">Mariages par Mois</div>
        <table>
            <thead>
                <tr>
                    <th>Mois</th>
            ...
            <tbody>
                @foreach($statistiques['parMois'] as $mois => $total)
                    <tr>
                        <td>{{ Carbon\Carbon::create()->month($mois)->locale('fr')->isoFormat('MMMM') }}</td>
                        <td>{{ $total }}</td>
                    </tr>
                @endforeach
            </tbody>
