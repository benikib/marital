    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('deces.attestation', $dece  ) }}" 
                   target="_blank"
                   class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 transition">
                    👁️ Voir attestation
                </a>
                
                {{-- <a href="{{ route('deces.attestation.pdf', $dece) }}"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex justify-between items-center flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    <div class="bg-sky-600 text-white px-4 py-2 rounded-lg font-bold">
                        Acte N° {{ str_pad($dece->id, 6, '0', STR_PAD_LEFT) }}
                    </div>
                    <div class="text-sm text-gray-600">
                        <span class="font-semibold">Enregistré le :</span> {{ $dece->created_at->format('d/m/Y à H:i') }}
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    @php
                        $numeroOfficiel = $dece->numero_acte ?? 'DEC-' . date('Y') . '-' . str_pad($dece->id, 4, '0', STR_PAD_LEFT);
                    @endphp
                    ...
                    <!-- Statut de l'acte -->
                    @php
                        $statutClass = match($dece->statut ?? 'enregistre') {
                            'enregistre' => 'bg-green-100 text-green-800',
                            'en_attente' => 'bg-yellow-100 text-yellow-800',
                            'annule' => 'bg-red-100 text-red-800',
                            default => 'bg-gray-100 text-gray-800'
                        };
                        $statutLabel = match($dece->statut ?? 'enregistre') {
                            'enregistre' => '✅ Enregistré',
                            'en_attente' => '⏳ En attente',
                            'annule' => '❌ Annulé',
                            default => '📄 ' . ($dece->statut ?? 'Enregistré')
                        };
                    @endphp
                ...
                <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                ...
                                <span class="font-semibold text-gray-800 text-lg">
                                    {{ $dece->personne->nom }} {{ $dece->personne->prenom }} 
                                    @if($dece->personne->postnom)
                                        <span class="text-gray-600 font-normal">{{ $dece->personne->postnom }}</span>
                                    @endif
                                </span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                ...
                                <span class="text-gray-700">
                                    {{ $dece->personne->date_naissance ? \Carbon\Carbon::parse($dece->personne->date_naissance)->format('d/m/Y') : 'Non renseignée' }}
                                </span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Heure de naissance :</span>
                                <span class="text-gray-700">{{ $dece->heure_naissance ?? 'Non renseignée' }}</span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Lieu de naissance :</span>
                                <span class="text-gray-700">{{ $dece->personne->lieu_naissance ?? $dece->lieu_naissance ?? 'Non renseigné' }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-500 font-medium">Sexe :</span>
                                <span class="px-3 py-1 {{ $dece->personne->sexe == 'M' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }} rounded-full text-sm font-medium">
                                    {{ $dece->personne->sexe == 'M' ? 'Masculin' : 'Féminin' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-4 bg-gradient-to-r from-gray-50 to-sky-50 rounded-xl border border-gray-200">
                            ...
                            <div class="space-y-2 text-sm">
                                <div class="flex items-start gap-2">
                                    ...
                                    <span class="font-medium text-gray-800">{{ $dece->etablissement ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Localité :</span>
                                    <span class="font-medium text-gray-800">{{ $dece->localite->nom ?? $dece->personne->lieu_naissance ?? 'Non renseignée' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Commune/Secteur :</span>
                                    <span class="font-medium text-gray-800">{{ $dece->commune ?? $dece->personne->secteur->nom ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Territoire :</span>
                                    <span class="font-medium text-gray-800">{{ $dece->territoire->nom ?? $dece->personne->territoire->nom ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Province :</span>
                                    <span class="font-medium text-gray-800">{{ $dece->province->nom ?? $dece->personne->province->nom ?? 'Non renseignée' }}</span>
                                </div>
                    ...
                    <div class="flex flex-col items-center justify-start">
                        ...
                        <div class="mt-4 w-full max-w-xs">
                            <div class="bg-sky-600 text-white rounded-lg p-3 text-center">
                                ...
                                <div class="text-xs mt-1 opacity-80">
                                    Déclaré le {{ $dece->date_declaration ? \Carbon\Carbon::parse($dece->date_declaration)->format('d/m/Y') : $dece->created_at->format('d/m/Y') }}
                                </div>
                            </div>
                        </div>

                        <!-- Mention marginale -->
                        @if($dece->mention_marginale)
                        <div class="mt-3 w-full max-w-xs bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                            ...
                            <p class="text-sm text-gray-700">{{ $dece->mention_marginale }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="font-semibold text-gray-800">
                                {{ $dece->pere_nom ?? $dece->personne->pere ?? 'Non renseigné' }}
                            </div>
                            @if($dece->pere_profession)
                                <div class="text-xs text-gray-500 mt-1">{{ $dece->pere_profession }}</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="font-semibold text-gray-800">
                                {{ $dece->mere_nom ?? $dece->personne->mere ?? 'Non renseignée' }}
                            </div>
                            @if($dece->mere_profession)
                                <div class="text-xs text-gray-500 mt-1">{{ $dece->mere_profession }}</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="pb-3 border-b border-gray-100">
                        ...
                        <p class="font-semibold text-gray-800 mt-1">
                            {{ $dece->declarant_nom ?? 'Non renseigné' }}
                        </p>
                        @if($dece->declarant_qualite)
                            <p class="text-sm text-gray-500">{{ $dece->declarant_qualite }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        ...
                        <span class="font-semibold">
                            {{ $dece->date_declaration ? \Carbon\Carbon::parse($dece->date_declaration)->format('d/m/Y') : $dece->created_at->format('d/m/Y') }}
                        </span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        <span class="text-gray-500 font-medium">Entité d'enregistrement :</span>
                        <span class="font-semibold">{{ $dece->entite->nom ?? 'Non définie' }}</span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        <span class="text-gray-500 font-medium">Officier d'état civil :</span>
                        <span class="font-medium">{{ $dece->officier_nom ?? $dece   ->user->name ?? 'Inconnu' }}</span>
                    </div>
                    <div class="flex justify-between items-center pt-2">
                        <span class="text-gray-500 font-medium">Dernière modification :</span>
                        <span class="text-sm text-gray-600">{{ $dece->updated_at->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white rounded-xl shadow p-4 border-l-4 border-blue-500">
                ...
                <p class="font-bold text-gray-800">  Acte N° {{ str_pad($dece->id, 6, '0', STR_PAD_LEFT) }}
</p>
            </div>
            <div class="bg-white rounded-xl shadow p-4 border-l-4 border-indigo-500">
                ...
                <p class="font-bold text-gray-800">{{ $dece->annee_enregistrement ?? date('Y') }}</p>
            </div>
        </div>

         @if ($dece->documents || $dece->certificat_medical || $dece->personne->photo)
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        @if ($dece->personne->photo)
                            <div class="group relative">
                                <div class="relative overflow-hidden rounded-lg bg-gray-100 aspect-square">
                                    <img src="{{ asset('storage/' . $dece->personne->photo) }}" alt="Photo"
                                        class="w-full h-full object-cover">
                                </div>
        ...
        <div class="mt-6 text-right text-xs text-gray-400">
            Acte dressé le {{ $dece->created_at->format('d/m/Y à H:i') }} 
            • Dernière mise à jour {{ $dece->updated_at->diffForHumans() }}
        </div>
    </div>
</x-app-layout>