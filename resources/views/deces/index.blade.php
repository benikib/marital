<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Décès</h2>
            <a href="{{ route('deces.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-white hover:bg-indigo-700">Nouveau Décès</a>
        </div>
    </x-slot>
        ...
        <div class="  ">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-2 text-gray-900">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($deces as $dece  )
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $dece->personne->nom ?? 'N/A' }} {{ $dece->personne->postnom ?? ' -' }} {{ $dece->personne->prenom ?? '-' }} {{ $dece->personne->date_naissance->format('d/m/Y') ?? ' -' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $dece->personne->lieu_naissance ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $dece->entite->nom  ?? 'N/A' }} ({{ ($dece->entite->type  ?? 'N/A') }}) </td>
                    ...
                    <div class="mt-4">{{ $deces->links() }}</div>
                </div>
            </div>
        </div>
    </div>
