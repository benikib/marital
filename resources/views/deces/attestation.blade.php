...
<body>
    ...
    <div class="acte">
        ...
        <div class="content">
            ...
            <div class="identity-block">
                <div class="identity-row">
                    ...
                    <span class="value">
                        <strong>{{ strtoupper($dece->personne->nom) }}</strong> 
                        {{ $dece->personne->prenom }}
                        @if($dece->personne->postnom)
                            {{ $dece->personne->postnom }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    <span class="label">Sexe :</span>
                    <span class="value">{{ $dece->personne->sexe == 'M' ? 'Masculin' : 'Féminin' }}</span>
                </div>
                <div class="identity-row">
                    <span class="label">Né(e) le :</span>
                    <span class="value">
                        {{ $dece->personne->date_naissance ? \Carbon\Carbon::parse($dece->personne->date_naissance)->translatedFormat('d F Y') : '_________' }}
                        @if($dece->heure_naissance)
                            à {{ $dece->heure_naissance }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    ...
                    <span class="value">
                        {{ $dece->personne->lieu_naissance ?? $dece->lieu_naissance ?? '_________________' }}
                        @if($dece->etablissement)
                            ({{ $dece->etablissement }})
                        @endif
                    </span>
                </div>
            ...
            <div class="parents-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $dece->pere_nom ?? $dece->personne->pere ?? '_________________' }}</span>
                </div>
                @if($dece->pere_profession)
                <div class="identity-row">
                    <span class="label">Profession :</span>
                    <span class="value">{{ $dece->pere_profession }}</span>
                </div>
                @endif
                <div class="identity-row" style="margin-top: 8px;">
                    <span class="label">Mère :</span>
                    <span class="value">{{ $dece->mere_nom ?? $dece->personne->mere ?? '_________________' }}</span>
                </div>
                @if($dece->mere_profession)
                <div class="identity-row">
                    <span class="label">Profession :</span>
                    <span class="value">{{ $dece->mere_profession }}</span>
                </div>
                @endif
            </div>
            ...
            <div class="declaration-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $dece->declarant_nom ?? '_________________' }}</span>
                </div>
                @if($dece->declarant_qualite)
                <div class="identity-row">
                    <span class="label">Qualité :</span>
                    <span class="value">{{ $dece->declarant_qualite }}</span>
                </div>
                @endif
                <div class="identity-row">
            ...
            <div style="display: flex; gap: 15px; margin: 8px 0; font-size: 11px;">
                <div>
                    ...
                    {{ $dece->type_naissance ?? 'Simple' }}
                </div>
                <div>
                    <span style="font-weight: 700;">Registre N° :</span> 
                    {{ $dece->numero_registre ?? '________' }}
                </div>
            </div>
            ...
            <div class="mention-box">
                <strong>📌 Mention marginale :</strong> {{ $dece->mention_marginale }}
            </div>
            @endif
            ...
            <div class="attestation-text">
                <p>
                    ...
                    <strong>{{ $dece->entite->nom ?? '_________________' }}</strong>.
                </p>
            </div>
        ...
        <div class="signature-section">
            ...
            <div class="signature">
                ...
                <div style="margin-top: 2px; font-size: 10px;">
                    {{ $dece->officier_nom ?? $dece->user->name ?? '_______________' }}
                </div>
                <div class="seal">
        ...
        <div class="qr-section">
            <div class="qr-code">
                <img width="65" height="65" 
                     src="https://api.qrserver.com/v1/create-qr-code/?size=75x75&data={{ urlencode(route('deces.verify', $dece)) }}&bgcolor=fffdf8&color=0284c7" 
                     alt="QR">
            </div>
            <div class="qr-text">
                ...
                <p style="margin-top: 3px; font-size: 7px;">
                    Réf : {{ $dece->numero_acte ?? 'DEC-' . $dece->id }}
                </p>
            </div>
        </div>
        ...
        <!-- Filigrane -->
        <div class="watermark">ACTE DE DÉCÈS</div>
    </div>
</body>
</html>