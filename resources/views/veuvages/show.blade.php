<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('veuvages.attestation', $veuvage) }}" 
                   target="_blank"
                ...
                <a href="{{ route('veuvages.edit', $veuvage) }}" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex justify-between items-center flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    <div class="bg-sky-600 text-white px-4 py-2 rounded-lg font-bold">
                        Acte N° {{ str_pad($veuvage->id, 6, '0', STR_PAD_LEFT) }}
                    </div>
                    <div class="text-sm text-gray-600">
                        <span class="font-semibold">Enregistré le :</span> {{ $veuvage->created_at->format('d/m/Y à H:i') }}
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    @php
                        $numeroOfficiel = $veuvage->numero_acte ?? 'VEU-' . date('Y') . '-' . str_pad($veuvage->id, 4, '0', STR_PAD_LEFT);
                    @endphp
                    ...
                    <!-- Statut de l'acte -->
                    @php
                        $statutClass = match($veuvage->statut ?? 'enregistre') {
                            'enregistre' => 'bg-green-100 text-green-800',
                            'en_attente' => 'bg-yellow-100 text-yellow-800',
                            'annule' => 'bg-red-100 text-red-800',
                            default => 'bg-gray-100 text-gray-800'
                        };
                        $statutLabel = match($veuvage->statut ?? 'enregistre') {
                            'enregistre' => '✅ Enregistré',
                            'en_attente' => '⏳ En attente',
                            'annule' => '❌ Annulé',
                            default => '📄 ' . ($veuvage->statut ?? 'Enregistré')
                        };
                    @endphp
        ...
        <!-- CARTE IDENTITÉ DU VEUFS/VEUVES -->
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-sky-600 to-blue-600 px-6 py-4">
                <h4 class="text-white font-bold text-lg flex items-center gap-2">
                    <span>👶</span> Identité du veuf/veuve
                </h4>
            </div>
            <div class="p-6">
                <div class="grid md:grid-cols-2 gap-6">
                    <!-- Informations du veuf/veuve -->
                    <div class="space-y-4">
                        <div class="space-y-3">
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Nom complet :</span>
                                <span class="font-semibold text-gray-800 text-lg">
                                    {{ $veuvage->personne->nom }} {{ $veuvage->personne->prenom }} 
                                    @if($veuvage->personne->postnom)
                                        <span class="text-gray-600 font-normal">{{ $veuvage->personne->postnom }}</span>
                                    @endif
                                </span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                ...
                                <span class="text-gray-700">
                                    {{ $veuvage->personne->date_naissance ? \Carbon\Carbon::parse($veuvage->personne->date_naissance)->format('d/m/Y') : 'Non renseignée' }}
                                </span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Heure de naissance :</span>
                                <span class="text-gray-700">{{ $veuvage->heure_naissance ?? 'Non renseignée' }}</span>
                            </div>
                            <div class="flex justify-between items-center pb-2 border-b border-gray-100">
                                <span class="text-gray-500 font-medium">Lieu de naissance :</span>
                                <span class="text-gray-700">{{ $veuvage->personne->lieu_naissance ?? $veuvage->lieu_naissance ?? 'Non renseigné' }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-gray-500 font-medium">Sexe :</span>
                                <span class="px-3 py-1 {{ $veuvage->personne->sexe == 'M' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }} rounded-full text-sm font-medium">
                                    {{ $veuvage->personne->sexe == 'M' ? 'Masculin' : 'Féminin' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-4 bg-gradient-to-r from-gray-50 to-sky-50 rounded-xl border border-gray-200">
                            ...
                            <div class="space-y-2 text-sm">
                                <div class="flex items-start gap-2">
                                    ...
                                    <span class="font-medium text-gray-800">{{ $veuvage->etablissement ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Localité :</span>
                                    <span class="font-medium text-gray-800">{{ $veuvage->localite->nom ?? $veuvage->personne->lieu_naissance ?? 'Non renseignée' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Commune/Secteur :</span>
                                    <span class="font-medium text-gray-800">{{ $veuvage->commune ?? $veuvage->personne->secteur->nom ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Territoire :</span>
                                    <span class="font-medium text-gray-800">{{ $veuvage->territoire->nom ?? $veuvage->personne->territoire->nom ?? 'Non renseigné' }}</span>
                                </div>
                                <div class="flex items-start gap-2">
                                    <span class="text-gray-500 min-w-28">Province :</span>
                                    <span class="font-medium text-gray-800">{{ $veuvage->province->nom ?? $veuvage->personne->province->nom ?? 'Non renseignée' }}</span>
                                </div>
                    ...
                    <div class="flex flex-col items-center justify-start">
                        @if($veuvage->personne->photo)
                            <div class="relative w-48 h-48 rounded-xl overflow-hidden shadow-lg border-2 border-gray-200">
                                <img src="{{ asset('storage/' . $veuvage->personne->photo) }}" 
                        ...
                        <div class="mt-4 w-full max-w-xs">
                            <div class="bg-sky-600 text-white rounded-lg p-3 text-center">
                                ...
                                <div class="text-xs mt-1 opacity-80">
                                    Déclaré le {{ $veuvage->date_declaration ? \Carbon\Carbon::parse($veuvage->date_declaration)->format('d/m/Y') : $veuvage->created_at->format('d/m/Y') }}
                                </div>
                            </div>
                        </div>

                        <!-- Mention marginale -->
                        @if($veuvage->mention_marginale)
                        <div class="mt-3 w-full max-w-xs bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                            ...
                            <p class="text-sm text-gray-700">{{ $veuvage->mention_marginale }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="font-semibold text-gray-800">
                                {{ $veuvage->pere_nom ?? $veuvage->personne->pere ?? 'Non renseigné' }}
                            </div>
                            @if($veuvage->pere_profession)
                                <div class="text-xs text-gray-500 mt-1">{{ $veuvage->pere_profession }}</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="font-semibold text-gray-800">
                                {{ $veuvage->mere_nom ?? $veuvage->personne->mere ?? 'Non renseignée' }}
                            </div>
                            @if($veuvage->mere_profession)
                                <div class="text-xs text-gray-500 mt-1">{{ $veuvage->mere_profession }}</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-3">
                    ...
                    <div class="pb-3 border-b border-gray-100">
                        ...
                        <p class="font-semibold text-gray-800 mt-1">
                            {{ $veuvage->declarant_nom ?? 'Non renseigné' }}
                        </p>
                        @if($veuvage->declarant_qualite)
                            <p class="text-sm text-gray-500">{{ $veuvage->declarant_qualite }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        ...
                        <span class="font-semibold">
                            {{ $veuvage->date_declaration ? \Carbon\Carbon::parse($veuvage->date_declaration)->format('d/m/Y') : $veuvage->created_at->format('d/m/Y') }}
                        </span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        <span class="text-gray-500 font-medium">Entité d'enregistrement :</span>
                        <span class="font-semibold">{{ $veuvage->entite->nom ?? 'Non définie' }}</span>
                    </div>
                    <div class="flex justify-between items-center pb-3 border-b border-gray-100">
                        <span class="text-gray-500 font-medium">Officier d'état civil :</span>
                        <span class="font-medium">{{ $veuvage->officier_nom ?? $veuvage->user->name ?? 'Inconnu' }}</span>
                    </div>
                    <div class="flex justify-between items-center pt-2">
                        <span class="text-gray-500 font-medium">Dernière modification :</span>
                        <span class="text-sm text-gray-600">{{ $veuvage->updated_at->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white rounded-xl shadow p-4 border-l-4 border-blue-500">
                ...
                <p class="font-bold text-gray-800">{{ $veuvage->numero_registre ?? 'Non attribué' }}</p>
            </div>
            <div class="bg-white rounded-xl shadow p-4 border-l-4 border-indigo-500">
                ...
                <p class="font-bold text-gray-800">{{ $veuvage->annee_enregistrement ?? date('Y') }}</p>
            </div>
        </div>

        <!-- SECTION PIÈCES JOINTES -->
        @if($veuvage->documents || $veuvage->certificat_medical || $veuvage->personne->photo)
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @if($veuvage->certificat_medical)
                        <div class="group relative">
                            <div class="relative overflow-hidden rounded-lg bg-gray-100 aspect-square">
                                <img src="{{ asset('storage/' . $veuvage->certificat_medical) }}" 
        ...
        <div class="mt-6 text-right text-xs text-gray-400">
            Acte dressé le {{ $veuvage->created_at->format('d/m/Y à H:i') }} 
            • Dernière mise à jour {{ $veuvage->updated_at->diffForHumans() }}
        </div>
    </div>
</x-app-layout>