...
<body>
    ...
    <div class="acte">
        ...
        <div class="header">
            ...
            <div class="numero">
                N° {{ $veuvage->numero_acte ?? 'VEUV-' . date('Y') . '-' . str_pad($veuvage->id, 5, '0', STR_PAD_LEFT) }}
            </div>
        </div>
        ...
        <div class="content">
            ...
            <!-- Identité du veuf/veuve -->
            <div class="identity-block">
                <div class="identity-row">
                    <span class="label">Nom et Prénoms :</span>
                    <span class="value">
                        <strong>{{ strtoupper($veuvage->personne->nom) }}</strong> 
                        {{ $veuvage->personne->prenom }}
                        @if($veuvage->personne->postnom)
                            {{ $veuvage->personne->postnom }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    <span class="label">Sexe :</span>
                    <span class="value">{{ $veuvage->personne->sexe == 'M' ? 'Masculin' : 'Féminin' }}</span>
                </div>
                <div class="identity-row">
                    <span class="label">Né(e) le :</span>
                    <span class="value">
                        {{ $veuvage->personne->date_naissance ? \Carbon\Carbon::parse($veuvage->personne->date_naissance)->translatedFormat('d F Y') : '_________' }}
                        @if($veuvage->heure_naissance)
                            à {{ $veuvage->heure_naissance }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    ...
                    <span class="value">
                        {{ $veuvage->personne->lieu_naissance ?? $veuvage->lieu_naissance ?? '_________________' }}
                        @if($veuvage->etablissement)
                            ({{ $veuvage->etablissement }})
                        @endif
                    </span>
                </div>
            </div>
            ...
            <div class="parents-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $veuvage->pere_nom ?? $veuvage->personne->pere ?? '_________________' }}</span>
                </div>
                @if($veuvage->pere_profession)
                <div class="identity-row">
                    <span class="label">Profession :</span>
                    <span class="value">{{ $veuvage->pere_profession }}</span>
                </div>
                @endif
                <div class="identity-row" style="margin-top: 8px;">
                    <span class="label">Mère :</span>
                    <span class="value">{{ $veuvage->mere_nom ?? $veuvage->personne->mere ?? '_________________' }}</span>
                </div>
                @if($veuvage->mere_profession)
                <div class="identity-row">
                    <span class="label">Profession :</span>
                    <span class="value">{{ $veuvage->mere_profession }}</span>
                </div>
                @endif
            </div>
            ...
            <div class="declaration-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $veuvage->declarant_nom ?? '_________________' }}</span>
                </div>
                @if($veuvage->declarant_qualite)
                <div class="identity-row">
                    <span class="label">Qualité :</span>
                    <span class="value">{{ $veuvage->declarant_qualite }}</span>
                </div>
                @endif
                <div class="identity-row">
            ...
            <div style="display: flex; gap: 15px; margin: 8px 0; font-size: 11px;">
                <div>
                    ...
                    {{ $veuvage->type_naissance ?? 'Simple' }}
                </div>
                <div>
                    <span style="font-weight: 700;">Registre N° :</span> 
                    {{ $veuvage->numero_registre ?? '________' }}
                </div>
            </div>
            ...
            <div class="mention-box">
                <strong>📌 Mention marginale :</strong> {{ $veuvage->mention_marginale }}
            </div>
            @endif
            ...
            <div class="attestation-text">
                <p>
                    ...
                    <strong>{{ $veuvage->entite->nom ?? '_________________' }}</strong>.
                </p>
            </div>
        ...
        <div class="signature-section">
            <div class="date-place">
                <p>Fait à {{ $veuvage->entite->nom ?? 'Kinshasa' }},</p>
            ...
            <div class="signature">
                ...
                <div style="margin-top: 2px; font-size: 10px;">
                    {{ $veuvage->officier_nom ?? $veuvage->user->name ?? '_______________' }}
                </div>
                <div class="seal">
        ...
        <div class="qr-section">
            <div class="qr-code">
                <img width="65" height="65" 
                     src="https://api.qrserver.com/v1/create-qr-code/?size=75x75&data={{ urlencode(route('veuvages.verify', $veuvage)) }}&bgcolor=fffdf8&color=0284c7" 
                     alt="QR">
            </div>
            <div class="qr-text">
                ...
                <p style="margin-top: 3px; font-size: 7px;">
                    Réf : {{ $veuvage->numero_acte ?? 'VEU-' . $veuvage->id }}
                </p>
            </div>
        </div>
