...
@section('content')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Délai pour éviter des requêtes à chaque frappe
            let debounceTimer;
            const debounceDelay = 800;

            // Fonction pour vérifier si une personne existe
            function checkPersonExists(fieldPrefix, type) {
                const nomInput = document.getElementById(`${fieldPrefix}_nom`);
                const prenomInput = document.getElementById(`${fieldPrefix}_prenom`);
                const dateNaissanceInput = document.getElementById(`${fieldPrefix}_date_naissance`);

                if (!nomInput || !prenomInput || !dateNaissanceInput) return;

                const nom = nomInput.value.trim();
                const prenom = prenomInput.value.trim();
                const dateNaissance = dateNaissanceInput.value;

                if (nom.length < 2 || prenom.length < 2 || !dateNaissance) return;

                fetch('/check-person-exists', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.exists) {
                            showAlert(`${type} est déjà enregistré(e) dans le système!`, 'warning');
                        }
                    })
                    .catch(error => {
            ...
            function showAlert(message, type) {
                // Supprimer les anciennes alertes du même type
                const oldAlerts = document.querySelectorAll(`.duplicate-alert.alert-${type}`);
                oldAlerts.forEach(alert => alert.remove());

                const alertDiv = document.createElement('div');
                alertDiv.className = `alert alert-${type} duplicate-alert alert-dismissible fade show`;
