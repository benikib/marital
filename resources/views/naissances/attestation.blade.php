...
<body>
    ...
    <div class="acte">
        ...
        <div class="header">
            ...
            <div class="numero">
                N° {{ $naissance->numero_acte ?? 'NAISS-' . date('Y') . '-' . str_pad($naissance->id, 5, '0', STR_PAD_LEFT) }}
            </div>
        </div>
        ...
        <div class="content">
            ...
            <div class="identity-block">
                <div class="identity-row">
                    ...
                    <span class="value">
                        <strong>{{ strtoupper($naissance->personne->nom) }}</strong> 
                        {{ $naissance->personne->prenom }}
                        @if($naissance->personne->postnom)
                            {{ $naissance->personne->postnom }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    <span class="label">Sexe :</span>
                    <span class="value">{{ $naissance->personne->sexe == 'M' ? 'Masculin' : 'Féminin' }}</span>
                </div>
                <div class="identity-row">
                    <span class="label">Né(e) le :</span>
                    <span class="value">
                        {{ $naissance->personne->date_naissance ? \Carbon\Carbon::parse($naissance->personne->date_naissance)->translatedFormat('d F Y') : '_________' }}
                        @if($naissance->heure_naissance)
                            à {{ $naissance->heure_naissance }}
                        @endif
                    </span>
                </div>
                <div class="identity-row">
                    ...
                    <span class="value">
                        {{ $naissance->personne->lieu_naissance ?? $naissance->lieu_naissance ?? '_________________' }}
                        @if($naissance->etablissement)
            ...
            <div class="parents-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $naissance->pere_profession }}</span>
                </div>
                @endif
                <div class="identity-row" style="margin-top: 8px;">
                    <span class="label">Mère :</span>
                    <span class="value">{{ $naissance->mere_nom ?? $naissance->personne->mere ?? '_________________' }}</span>
                </div>
                @if($naissance->mere_profession)
                <div class="identity-row">
                    <span class="label">Profession :</span>
                    <span class="value">{{ $naissance->mere_profession }}</span>
                </div>
                @endif
            </div>
            ...
            <div class="declaration-block">
                ...
                <div class="identity-row">
                    ...
                    <span class="value">{{ $naissance->declarant_qualite }}</span>
                </div>
                @endif
                <div class="identity-row">
            ...
            <div class="attestation-text">
                <p>
                    ...
                    <strong>{{ $naissance->entite->nom ?? '_________________' }}</strong>.
                </p>
            </div>
        ...
        <div class="signature-section">
            <div class="date-place">
                <p>Fait à {{ $naissance->entite->nom ?? 'Kinshasa' }},</p>
            ...
            <div class="signature">
                ...
                <div style="margin-top: 2px; font-size: 10px;">
                    {{ $naissance->officier_nom ?? $naissance->user->name ?? '_______________' }}
                </div>
                <div class="seal">
        ...
        <div class="qr-section">
            <div class="qr-code">
                <img width="65" height="65" 
                     src="https://api.qrserver.com/v1/create-qr-code/?size=75x75&data={{ urlencode(route('naissances.verify', $naissance)) }}&bgcolor=fffdf8&color=0284c7" 
                     alt="QR">
            </div>
            <div class="qr-text">
                ...
                <p style="margin-top: 3px; font-size: 7px;">
                    Réf : {{ $naissance->numero_acte ?? 'NAISS-' . $naissance->id }}
                </p>
            </div>
        </div>
